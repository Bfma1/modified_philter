...
###########################
    ...
    def adjust_date_length(self, original, shifted):
        """
        Adjusts the shifted date string to match the length of the original date string.
        If shifted is shorter than original, pads it (e.g. with spaces).
        If shifted is longer than original, truncates it.
        Returns the adjusted string.
        """
        target_length = len(original)
        if len(shifted) < target_length:
            return shifted.ljust(target_length)
        elif len(shifted) > target_length:
         return shifted[:target_length]
        return shifted



    def shift_date(self, date_str, shift_days=None):
        """
        Given a date string, parse it and randomly shift it by 1 to 60 days (if shift_days is not provided).
        Returns the shifted date in ISO format (YYYY-MM-DD).
        If parsing fails, returns the original date_str.
        """
        try:
            # Parse the date (fuzzy=True allows extra text)
            dt = parse(date_str, fuzzy=True)
            if shift_days is None:
                shift_days = random.randint(1, 60)
    ...
    def transform(self, allowed_filetypes=set(["txt", "ano"])):
        """ transform
            Turns input files into output PHI files. Protected health information will be replaced 
            by the replacement character.

            Transforms the data. Order is preserved, i.e., patterns at index 0 will have priority over 
            patterns at index 2.
        ...
        # Check allowed filetypes
            ...
            # Read the file contents with detected encoding
            encoding = self.detect_encoding(filename)
            txt = open(filename, "r", encoding=encoding['encoding']).read()

            # Prepare the output file base path
            fbase, fext = os.path.splitext(f)
            outpathfbase = os.path.join(out_path, fbase)

            if self.outformat == "asterisk":
            # First, generate the standard transformed text.
                contents = self.transform_text_asterisk(txt, filename)

           
            
            
            # Now, post-process to shift date PHI.
            # Assume that date PHI have been recorded in the phi_type dictionary under key "DATE"
                if "DATE" in self.phi_type_dict:
                    date_map = self.phi_type_dict["DATE"][0]
                # Generate a single random shift value for this note.
                    note_shift = random.randint(1, 60)
                # Loop through all recorded coordinates for dates in this file.
                    for start, stop in date_map.filecoords(filename):
                # Extract the original date substring from the original text.
                        original_date = txt[start:stop]
                # Compute the shifted date using the same shift for all dates in this note.
                        shifted_date = self.shift_date(original_date, note_shift)
                # Adjust the shifted date to match the original length:
                        shifted_date = self.adjust_date_length(original_date, shifted_date)
            # Replace the corresponding region in the output text.
                        contents = contents[:start] + shifted_date + contents[stop:]

                with open(outpathfbase + ".txt", "w", encoding='utf-8', errors='surrogateescape') as f_out:
                 f_out.write(contents)


            elif self.outformat == "i2b2":
                with open(outpathfbase + ".xml", "w", errors='xmlcharrefreplace') as f_out:
                    contents = self.transform_text_i2b2(self.data_all_files[filename])
                    f_out.write(contents)
            else:
                raise Exception("Outformat not supported: ", self.outformat)        

    # If evaluation data is needed, output it.
        if self.run_eval: 
    ...
    def eval(self,
        ...
        for root, dirs, files in os.walk(in_path):
            ...
            for f in files:
                ...
                # Read the final (shifted) text using the detected encoding
                encoding1 = self.detect_encoding(philtered_filename)
                final_text = open(philtered_filename, "r", encoding=encoding1['encoding']).read()

                # Use your existing get_clean method to tokenize the final text consistently.
                # This returns a list of tokens (cleaned and split as defined in get_clean).
                cleaned_tokens = self.get_clean(philtered_filename, final_text)

                # Compute start coordinates for each token in the final text.
                # This assumes that tokens appear sequentially and there are no changes in the final text.
                pos_coords = []
                current = 0
                for token in cleaned_tokens:
                    pos_coords.append(current)
                    current += len(token)

                # Generate POS tags for the tokenized final text.
                pos_list = nltk.pos_tag(cleaned_tokens)

                # Build a dictionary mapping each token's start coordinate (as a string) to its [word, POS] pair.
                cleaned_with_pos = {}
               ...
               # philtered_words_cleaned = self.get_clean(original_filename, philtered)
                ...
                for c,w,r in self.seq_eval(cleaned_tokens, anno_words_cleaned, f):
        ...
        for fn in summary_coords['summary_by_file']:
            ...
            if current_fps != [] and current_fps != [""]:              
                ...
                for word in current_fps:
                    ...
                    pos_entry = self.get_closest_pos_entry(cleaned_with_pos, start_coordinate_fp)
